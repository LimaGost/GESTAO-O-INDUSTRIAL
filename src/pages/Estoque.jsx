...
export default function Estoque() {
  ...
  return (
    ...
    <div className="space-y-5">
      ...
      {/* Pedidos aguardando confirmação de reserva de estoque (sem pagamento antecipado) */}
      {podeReservar && pedidosPendentes.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4">
          <div className="flex items-center gap-2 mb-3">
            <ClipboardList size={16} className="text-amber-700" />
            <h3 className="text-sm font-bold text-amber-800">Pedidos aguardando reserva de estoque ({pedidosPendentes.length})</h3>
          </div>
          <div className="space-y-2">
            {pedidosPendentes.map(pedido => (
              <div key={pedido.id} className="bg-white border border-amber-200 rounded-xl px-4 py-2.5 flex items-center justify-between gap-3 flex-wrap">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-foreground truncate">{pedido.cliente_nome} · {pedido.numero}</p>
                  <p className="text-xs text-muted-foreground">{(pedido.itens || []).length} item(ns)</p>
                </div>
                <button
                  onClick={() => confirmarReservaPedido(pedido)}
                  disabled={confirmandoPedidoId === pedido.id}
                  className="flex items-center gap-1.5 text-xs bg-emerald-600 text-white px-3 py-2 rounded-lg font-semibold hover:bg-emerald-700 transition-colors disabled:opacity-50 flex-shrink-0">
                  <Check size={13} /> {confirmandoPedidoId === pedido.id ? 'Confirmando...' : 'Confirmar Reserva'}
                </button>
              </div>
            ))}
          </div>
